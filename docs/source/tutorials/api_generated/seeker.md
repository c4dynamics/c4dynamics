...
The `seeker` class models sensors that
...
=================
bias_std : float
    The standard deviation of the bias error, [radians]. Defaults $0.1°$.
...
noise_std : float
    ...
    Default value for non-ideal seeker: $0.4°$.
dt : float
    The time-constant of the operational rate of the seeker
    (below which the seeker measures return None), [seconds]. Default value: $dt = -1sec$
...
using the `bias` property and determine the seeker bias error:
...
- The scale factor error increases with the angle, such that for a $5%$
  scale factor,
  the error of $Azimuth = 100°$ is $5°$, whereas the error for
  $Elevation = -15°$ is only $-0.75°$.
...
The seeker origin is yawing (performed by the increment of $\psi$)
...
```python
measure(self, target: 'c4d.state', t: float = -1, store: bool = False) -> tuple[typing.Optional[float], typing.Optional[float]]
```
